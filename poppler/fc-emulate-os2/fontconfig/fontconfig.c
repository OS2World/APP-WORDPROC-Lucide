...
//#define MATCH_DEBUG
  ...
  if (!pFont)
  {
    ...
    {
      strncpy(achKey, "DejaVu Sans", sizeof(achKey));
    }
...
fcExport FcFontSet *FcFontSort(FcConfig *config, FcPattern *p, FcBool trim, FcCharSet **csp, FcResult *result)
{
  ...
  // enhanced for poppler usage.
  // according to docs fcfontsort brings back the fonts matching the pattern. so we use FcFontMatch 
  // if FcFontMatch has no font found we try the default ones
  ...
  *result = FcResultMatch;
  ...
  if (!newPattern)
  {
     ...
     if (p->lang && (stristr(p->lang, "JA") != NULL))
        {
           FcPatternAddString(pDup, FC_FAMILY, DEFAULT_SERIF_FONTJA);
        }
        else
        {
           FcPatternAddString(pDup, FC_FAMILY, DEFAULT_SERIF_FONT);
        }
