...
static VOID showPopup( toolbarWndStruct *tws, HWND hwnd, MPARAM mp1, twsItem *ti )
{
    ...
    {
        tws->mshowed = TRUE;
        SHORT id = (SHORT)WinSendMsg( ti->hmenu, MM_ITEMIDFROMPOSITION,
...
static MRESULT EXPENTRY toolbarProc( HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2 )
{
    ...
    switch ( msg )
    {
        ...
        case TBM_QUERYCHECK:
            return (MRESULT)tws->queryCheck( (SHORT)mp1 );

        case TBM_SETCHECK:
            tws->setCheck( (SHORT)mp1, (BOOL)mp2 );
            WinInvalidateRect( hwnd, NULL, TRUE );
            return (MRESULT)(TRUE);

        case TBM_ENABLEITEM:
            tws->enableItem( (SHORT)mp1, (BOOL)mp2 );
            WinInvalidateRect( hwnd, NULL, TRUE );
            return (MRESULT)(TRUE);

        case TBM_SETBUBBLETEXT:
            tws->setBubbleText( (SHORT)mp1, (PCSZ)mp2 );
            return (MRESULT)(TRUE);
