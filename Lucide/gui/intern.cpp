...
static void loadLng( map<string,string> *array, const char *file, bool exitOnError )
{
    ...
    else
    {
        ...
        while ( !lngFile.eof() )
        {
            // @todo this code will treat the rest of the line which is longer
            // than LINEBUF_LEN-1 chars as a new line which is wrong
            lngFile.getline( line, LINEBUF_LEN );
            if ( ( line[ 0 ] != '#' ) && ( line[ 0 ] != ' ' ) )
            {
                char *eqpos = NULL;
...
#else
    _execname( appdir, sizeof(appdir) );
    _splitpath( appdir, drive, dir, NULL, NULL );
#endif
...
void localizeMenu( HWND hmenu )
{
    ...
    SHORT id, len;
    SHORT cnt = SHORT1FROMMR( WinSendMsg( hmenu, MM_QUERYITEMCOUNT, MPVOID, MPVOID ) );
    for ( SHORT i = 0; i < cnt; i++ )
    {
        id = SHORT1FROMMR( WinSendMsg( hmenu, MM_ITEMIDFROMPOSITION, MPFROMSHORT( i ), MPVOID ) );

        len = SHORT1FROMMR( WinSendMsg( hmenu, MM_QUERYITEMTEXT, MPFROM2SHORT( id, mtext.getSize() ),
