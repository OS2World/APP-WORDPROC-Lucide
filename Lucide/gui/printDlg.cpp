...
bool PrintDlg::isPostscriptDevice()
{
    ...
    if ( hdcPrinterInfo != DEV_ERROR ) {
        DevQueryCaps( hdcPrinterInfo, CAPS_TECHNOLOGY, 1, &lTech );
        ...
        case WM_CONTROL:
        {
            switch ( SHORT1FROMMP(mp1) )
            {
                case IDC_PNAME:
                {
                    ...
                    {
                        SHORT rc = SHORT1FROMMR( WinSendDlgItemMsg( hwnd, IDC_PNAME, LM_QUERYSELECTION,
                ...
                case DID_OK:
                    {
                        ...
                        {
                            SHORT rc = SHORT1FROMMR( WinSendDlgItemMsg( hwnd, IDC_RANGE_EVEN_ODD, LM_QUERYSELECTION,
                                                                        MPFROMSHORT( LIT_CURSOR ), MPVOID ) );
                            if ( rc != LIT_NONE ) {
