...
extern "C" char * EXPENTRY getDescription()
{
    ...
    strcat(version, POPPLER_VERSION);
    return version;
}
...
static void copy_page_to_pixbuf( Environment *ev, SplashBitmap *bitmap, LuPixbuf *pixbuf )
{
    ...
    width = std::min( splash_width, pixbuf_width );
    height = std::min( splash_height, pixbuf_height );
    rowstride = std::min( splash_rowstride, pixbuf_rowstride );
                                   ...
                                   Environment *ev, string filename)
{
    ...
    if ( filename != NULL ) {
        retval = document->doc->saveAs( new GooString( filename ) ) == errNone;
    }
                                                               ...
                                                               Environment *ev)
{
    ...
    for ( int i = 0; i < len; i++ )
    {
        ...
        // SubstituteName
        GooString *ggnm = info->getSubstituteName();
        if ( ggnm != NULL )
        {
            fonts->_buffer[i].subname = somstrdup( ggnm->getCString() );
                   }
    }
...
static char *propcnv( GooString *s, void *objUniBe, void *objSys )
{
    ...
    {
        char *str = uniUtf16BEToSys( s->getCString(), s->getLength(), &objUniBe,
                                     &objSys );
        char *ret = somstrdup( str );
        delete[] str;
        return ret;
    }
                                                    ...
                                                    Environment *ev)
{
    ...
    Object objdict = doc->getDocInfo();
    if ( objdict.isDict() )
    {
        Dict *d = objdict.getDict();
        Object obj = d->lookup( "Title");
...
SOM_Scope boolean  SOMLINK isHaveInputFields(LuPopplerDocument *somSelf,
                                              Environment *ev)
{
    LuPopplerDocumentData *somThis = LuPopplerDocumentGetData(somSelf);
...
SOM_Scope LuDocument_LuInputFieldSequence*  SOMLINK getInputFields(LuPopplerDocument *somSelf,
                                                                    Environment *ev,
                                                                   long pagenum)
{
    LuPopplerDocumentData *somThis = LuPopplerDocumentGetData(somSelf);
    ...
    int count;
    if (widgets == NULL || (count = widgets->getNumWidgets()) <= 0) {
        fields->_maximum = 0;
        fields->_length = 0;
        fields->_buffer = NULL;
        return fields;
    ...
    fields->_maximum = count;
    fields->_length = count;
    fields->_buffer = (LuInputField **)SOMMalloc( sizeof( LuInputField * ) * count );

    for (int i = 0; i < widgets->getNumWidgets(); ++i) {
        FormWidget *widget = widgets->getWidget( i );
        switch( widget->getType() ) {
            case formButton:
                fields->_buffer[ i ] = static_cast< LuPopplerInputField * >(
                    new LuPopplerInputButton( ev, somSelf, pagenum, page,
                        static_cast< FormWidgetButton * >( widget ) ) );
                break;
            case formText:
                fields->_buffer[ i ] = static_cast< LuPopplerInputField * >(
                    new LuPopplerInputText( ev, somSelf, pagenum, page,
                        static_cast< FormWidgetText * >( widget ) ) );
                break;
#if 1
            case formChoice:
                fields->_buffer[ i ] = static_cast< LuPopplerInputField * >(
                       new LuPopplerInputChoice( ev, somSelf, pagenum, page,
                        static_cast< FormWidgetChoice * >( widget ) ) );
                break;
#endif
#if 0
            case formSignature:
                fields->_buffer[ i ] = static_cast< LuPopplerInputField * >(
                    new LuPopplerInputSignature( ev, somSelf, pagenum, page,
                        static_cast< FormWidgetSignature * >( widget ) ) );
                break;
#endif
            default:
                fields->_buffer[ i ] =
                    new LuPopplerInputField( ev, somSelf, pagenum, page, widget );
                break;
        }
    }
                                      ...
                                      som3InitCtrl* ctrl)
{
    ...
    LuPopplerDocumentData *somThis = NULL;
